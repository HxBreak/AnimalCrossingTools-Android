...
Java_com_hxbreak_animalcrossingtools_jni_NativeTurnip_calculate(JNIEnv *env, jobject thiz,
                                                                jint pattern, jint basePrice, jintArray array) {
    TurnipPrices turnips;
    turnips.rng.init();
    turnips.basePrice = basePrice;
    turnips.whatPattern = pattern;
    jboolean copy = 0;
    auto p = env->GetIntArrayElements(array, &copy);
    for (int i = 0; i < 14 ; ++i) {
        turnips.sellPrices[i] = p[i];
    }
    env->ReleaseIntArrayElements(array, p, NULL);
    turnips.calculate();
    for (int i = 0; i < 14 ; ++i) {
        env->SetIntArrayRegion(array, 0, 14, turnips.sellPrices);
    }
}