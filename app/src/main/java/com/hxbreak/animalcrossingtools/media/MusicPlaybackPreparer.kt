...
class MusicPlaybackPreparer(
    private val mediaList: List<MediaMetadataCompat>,
...
//                PlaybackStateCompat.ACTION_PLAY_FROM_SEARCH or
    ...
    override fun onPrepareFromMediaId(mediaId: String, playWhenReady: Boolean, extras: Bundle?) {
        val itemToPlay: MediaMetadataCompat? = mediaList.find { item ->
            item.id == mediaId
        }
        if (itemToPlay == null) {
    ...
    override fun onPrepareFromUri(uri: Uri, playWhenReady: Boolean, extras: Bundle?) {
        val tag = extras?.getParcelable<MediaMetadataCompat>("MediaMetaData")
    ...
    private fun buildPlaylist(item: MediaMetadataCompat): List<MediaMetadataCompat> =
        mediaList.filter { it.album == item.album }.sortedBy { it.trackNumber }
}
